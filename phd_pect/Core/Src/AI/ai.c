...
//
//  int8_t class = 0;
//  float max_output = m_output[class];
//  for (int i = 0; i < AI_NETWORK_OUT_1_SIZE; i++) {
//    if (m_output[i] > max_output) {
//      class = i;
//      max_output = m_output[i];
//    }
//  }
//
//  return class;
//}
int8_t ai_run(const float* input, uint32_t len, int8_t* output)
{
  if (len < AI_NETWORK_IN_1_SIZE) {
    return -1;
  }
